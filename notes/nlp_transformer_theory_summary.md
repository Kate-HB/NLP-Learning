...
**序列标注（Sequence Labeling）**为序列中的每个词或 Token 预测标签。Token 是模型实际处理的文本单位，后文第 3 节会详细解释。典型任务包括命名实体识别（NER，识别人名、地名等实体）、词性标注（POS，判断名词、动词等语法类别）和句法分块（Chunking，识别名词短语等局部结构）。
...
**梯度（Gradient）**表示参数发生微小变化时，损失会怎样变化。梯度指出让损失增长最快的方向，因此沿反方向更新参数可降低损失。
